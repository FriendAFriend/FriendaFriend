...
router.get('/dashboard', listingController.getAllListings, (req, res) => {
  res.status(200).json(res.locals.allListings);
});

// page for only the user's own listings
router.get('/myListings', listingController.getListing, (req, res) => {
...
router.put('/updateListing', listingController.updateListing, (req, res) => {
  // res.redirect('/dashboard');
  res.status(200).json(res.locals.listing);
});

router.post('/newPhoto', photoController.createPhoto, (req, res) => {
